...
const TRIVIAL_SIL: &str = include_str!(concat!(env!("CARGO_MANIFEST_DIR"), "/examples/trivial.sil"));

#[test]
fn trivial_sil_compiles_to_nonempty_script_and_p2sh() {
    // SilverScript (faaa074): compile the template to redeem-script bytes.
    let compiled = compile_contract(TRIVIAL_SIL, &[], CompileOptions::default()).expect("trivial.sil must compile");
    ...
    let recomputed = Params::new().hash_length(32).to_state().update(redeem).finalize();
    assert_eq!(p2sh_hash, recomputed.as_bytes(), "P2SH must commit to blake2b-256(redeem)");
}
